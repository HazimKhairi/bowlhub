...
class TeamBerkumpulanImport extends ParticipantImport
{
    /**
    ...
    protected function getTeamMembers(array $row): array
    {
        ...
        if (! empty($row['member_4_nama'])) {
            $members[4] = [
                'name' => $row['member_4_nama'],
                'ic' => $row['member_4_kp'] ?? '',
            ];
        }

        if (! empty($row['member_5_nama'])) {
            $members[5] = [
                'name' => $row['member_5_nama'],
                'ic' => $row['member_5_kp'] ?? '',
            ];
        }

        if (! empty($row['member_6_nama'])) {
            $members[6] = [
                'name' => $row['member_6_nama'],
                'ic' => $row['member_6_kp'] ?? '',
            ];
        }
    ...
    /**
     * Get the validation rules for team berkumpulan import.
     */
    public function rules(): array
    {
        $rules = parent::rules();

        // Team berkumpulan import requires ketua_kp, ketua_nama, and ketua_telefon
